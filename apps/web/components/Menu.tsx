    ...
    const fetchContinuableGames = async () => {
        ...
        try {
            ...
            const continuableGames = gamesData.filter(
                game => game.status === "IN_PROGRESS" || game.status === "NOT_STARTED",
            );
            ...
            const gamesWithCharacters = await Promise.all(
                continuableGames.map(async game => {
                    try {
                        const charResponse = await api.get(`/character/${game.characterId}`);
                ...
                <div className="p-4 md:p-6">
                    ...
                    <div className="flex items-center gap-3 mb-4 md:mb-6">
                        <div className="p-2 bg-accent/10 rounded-lg border border-accent/20">
                            <Sparkles size={18} className="text-accent md:w-5 md:h-5" />
                        </div>
                        <div>
                            <h2 className="text-base md:text-lg font-bold tracking-wide text-foreground">MAIN MENU</h2>
                    ...
                    <button
                        ...
                        className="w-full p-3 md:p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg hover:bg-yellow-500/20 transition-all duration-200 text-left group flex items-center gap-3 md:gap-4 mb-3"
                    >
                        <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-card border border-yellow-500/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                            <RefreshCw size={20} className="text-yellow-400 md:w-6 md:h-6" />
                        </div>
                        <div className="flex-1">
                            <h3 className="font-bold tracking-wide text-yellow-400 text-xs md:text-sm">CONTINUE</h3>
                    ...
                    <div className="space-y-2 md:space-y-3">
                        {menuItems.map(item => (
                            <button
                                ...
                                className={`w-full p-3 md:p-4 ${item.bgColor} border ${item.borderColor} rounded-lg ${item.hoverBg} transition-all duration-200 text-left group flex items-center gap-3 md:gap-4`}
                            >
                                <div
                                    className={`w-10 h-10 md:w-12 md:h-12 rounded-lg bg-card border ${item.borderColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
                                >
                                    <item.icon size={20} className={`${item.color} md:w-6 md:h-6`} />
                                </div>
                                <div className="flex-1">
                                    <h3 className={`font-bold tracking-wide ${item.color} text-xs md:text-sm`}>
                                        {item.name}
                                    </h3>
                                    <p className="text-[10px] md:text-xs text-foreground/50 mt-0.5">
                                        {item.description}
                    ...
                    <div className="relative w-full max-w-lg bg-card rounded-lg border border-border shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold tracking-wide text-foreground">CONTINUE GAME</h2>
                            ...
                            {loadingGames ? (
                                <div className="py-12 flex flex-col items-center gap-4">
                                    ...
                                    <span className="text-xs tracking-widest text-foreground/50">LOADING GAMES...</span>
                                </div>
                            ) : (
                                <div className="space-y-3 max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent pr-2">
                                    {games.length === 0 ? (
                                        <div className="text-center py-12 text-foreground/50 text-sm">
                                            ...
                                            <p className="font-bold tracking-wide">No games to continue</p>
                                            <p className="text-xs mt-2">Start a new adventure first</p>
                                        </div>
                                    ) : (
                                        games.map(game => (
                                            <button
