...
const TitleBar = ({ title, onBack }: { title: string; onBack?: () => void }) => (
    <div className="h-9 bg-[#1e2538] border-b border-border flex items-center justify-between px-3 select-none shrink-0">
        ...
        <div className="w-12 flex justify-end">
            {onBack && (
                <button onClick={onBack} className="p-1 hover:bg-card rounded transition-colors">
...
export default function CreateGameWindow() {
    ...
    useEffect(() => {
        const checkApiKey = async () => {
            try {
                const response = await api.get("/profile/has-api-key");
    ...
    const handleStartGame = async () => {
        if (!hasApiKey) {
            setError("API key is required. Please add your API key in your profile.");
            setErrorType("API_KEY_REQUIRED");
        ...
        try {
            ...
            if (er instanceof AxiosError) {
                ...
                setError(er.response?.data.message || "Failed to create game");
            ...
            <div className="flex flex-col h-screen overflow-hidden font-mono">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-4 animate-in fade-in duration-500">
                        ...
                        <span className="text-sm tracking-widest text-foreground/70">LOADING CHARACTERS...</span>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="flex-1 flex items-center justify-center p-4 overflow-auto">
                    <div className="w-full max-w-2xl bg-card rounded-lg border border-border shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="p-6">
                            ...
                            {/* API Key Warning */}
                            {hasApiKey === false && (
                                <div className="mb-4 px-4 py-3 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="flex items-center gap-3">
                                        <Key size={16} className="text-amber-400 shrink-0" />
                                        <span className="text-xs text-amber-400">API key required to start a game</span>
                                    </div>
                                    <button
                                        onClick={() => router.push("/profile")}
                                        className="px-3 py-1.5 bg-amber-500/20 border border-amber-500/30 rounded-lg text-xs font-bold text-amber-400 hover:bg-amber-500/30 transition-all"
                                    >
                                        Add Key
                                    </button>
                                </div>
                            ...
                            <div className="space-y-3 max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent pr-2">
                                {characters.length === 0 ? (
                                    <div className="text-center py-8 text-foreground/50 text-sm">
                                        ...
                                        <p className="text-xs mt-1">Create your first character to begin</p>
                                    </div>
                                ) : (
                    ...
                    <div className="w-full max-w-lg bg-card rounded-lg border border-border shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold tracking-wide text-foreground">START ADVENTURE</h2>
                                    <p className="text-xs text-foreground/50 uppercase tracking-widest">
                            ...
                            {hasApiKey === false && (
                                <div className="mb-4 px-4 py-3 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="flex items-center gap-3">
                                        <Key size={16} className="text-amber-400 shrink-0" />
                                        <span className="text-xs text-amber-400">API key required to start</span>
                                    </div>
                                    <button
                                        onClick={() => router.push("/profile")}
                                        className="px-3 py-1.5 bg-amber-500/20 border border-amber-500/30 rounded-lg text-xs font-bold text-amber-400 hover:bg-amber-500/30 transition-all"
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3 text-xs text-foreground/50 mt-1">
                                            <span className="capitalize">{selectedCharacterData.race}</span>
                                            <span>•</span>
                                            <span className="capitalize">{selectedCharacterData.gender}</span>
                                            <span>•</span>
